...
def create_allure_files():
    ...
    def _create(directory):
        ...
        skipped_file.write_text(json.dumps({"status": "skipped", "fullName": "test1"}))

        magic_file = directory / "magic-result.json"
        magic_file.write_text(
            json.dumps({"status": "skipped", "fullName": "features/_common/login"})
        )

        passed_file = directory / "passed-result.json"
        passed_file.write_text(json.dumps({"status": "passed", "fullName": "test2"}))
