...
def test_before_all_initialises_framework(
    mock_parse, mock_factory, mock_report, mock_loader, mock_context
):
...
def test_before_scenario_skips_common_library_on_bulk_run(mock_context, mock_scenario):
    ...
    expected_msg = (
        f"Skipping Library Scenario '{mock_scenario.name}' (Implicit Bulk Run)"
    )
    mock_scenario.skip.assert_called_with(expected_msg)


def test_before_scenario_runs_common_if_explicitly_targeted(
    mock_context, mock_scenario
):
